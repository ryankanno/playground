...
Sends the contents of http://www.espn.com to friend@domain.com using your
Gmail settings
...
def _validate_options_and_args(parser, options, args):
    ...
    if (len(args) != 1):
        parser.error("Incorrect number of arguments. \
            Script expects 1 (URL to backup), but received %i." % len(args))
        sys.exit(2)  # Command line syntax error
    elif not options.recipients:
        parser.error("You must include at least one recipient.")
        sys.exit(1)
    elif (options.username and options.password is None) or \
            (options.username is None and options.password is not None):
        ...
        for link in links:
            try:
                ...
                if href is not None and not "http://" in href:
                    link[link_attribute] = urlparse.urljoin(url,
                                                            cgi.escape(href))
            except KeyError:
                pass
...
def main():
    ...
    parser.add_option("-u", "--username", dest="username", metavar="USER",
                      help="Username to SMTP server")
    parser.add_option("-p", "--password", dest="password", metavar="PWD",
                      help="Password to SMTP server")
    parser.add_option("-s", "--server", dest="server", metavar="SERVER",
                      help="SMTP server (Defaults to Gmail)")
    parser.add_option("--port", dest="port", metavar="PORT", type="int",
                      help="SMTP server port (Defaults to Gmail)")
    parser.add_option("-f", "--from", dest="from_email", metavar="FROM",
                      help="From address")
    parser.add_option("-r", "--recipient", action="append", dest="recipients",
                      metavar="RCPT", type="string", help="Email recipient")
    parser.add_option('-a', '--attach', action='store_true',
                      dest='as_attachment', metavar="ATTACH",
                      help='As attachment')
    parser.add_option('-t', '--test', action="store_true", dest="test",
                      metavar="TEST", help="Run tests")
    parser.add_option('-v', '--verbose', action='store_const',
                      dest='log_level', const=logging.DEBUG,
                      help='Verbose output')
    parser.set_defaults(server="smtp.gmail.com",
                        port=587, test=False, log_level=logging.INFO)
    ...
    mail(options.from_email, options.recipients,
         '%s @ %s' % (args[0], (datetime.datetime.now().
                      strftime("%A %B %d %I:%M:%S %p %Y"))),
